...
DEFAULT_OUTPUT_ROOT = Path(__file__).resolve().parent / "output" / "candidate_selection_portfolio_case_study"
...
PORTFOLIO_NAME = "milestone_15_candidate_driven_case_study"
...
def _write_summary(
    ...
    *,
    ...
    approved_ids = selected_candidates["candidate_id"].astype("string").tolist()
    considered_ids = candidate_universe["candidate_id"].astype("string").tolist()

    summary = {
        "workflow": [
            "load_evaluated_alpha_sleeves",
            "compare_registry_backed_alpha_runs",
        ...
        "candidate_selection": {
            ...
            "thresholds": {
                "eligibility": candidate_result.thresholds,
                "redundancy": candidate_result.redundancy_thresholds,
                "allocation_constraints": candidate_result.allocation_constraints,
            },
            "selection_summary": {
                "rejected_candidates_by_stage": selection_summary.get("rejected_candidates_by_stage"),
                "allocation_summary": selection_summary.get("allocation_summary"),
            },
        ...
        "portfolio": {
            ...
            "candidate_selection_source": portfolio_result.config.get("candidate_selection"),
        },
        "review": {
        ...
        "artifacts": {
            "comparison": {
                "csv": _relative_to_output(comparison_result.csv_path, output_root),
                "json": _relative_to_output(comparison_result.json_path, output_root),
            },
...
def print_summary(summary: dict[str, Any], output_root: Path) -> None:
    print("Candidate Selection Portfolio Case Study")
    print(
        "Candidate inputs: "
        f"{summary['candidate_inputs']['considered_candidate_count']} candidates "
        f"(comparison_id={summary['candidate_inputs']['comparison_id']})"
    )
    ...
    print(
        "Allocation: "
        f"method={summary['allocation']['method']}, "
        f"weight_sum={summary['allocation']['weight_sum']:.6f}"
    )
    print(
        "Portfolio: "
    ...
    print(
        "Review: "
        f"selected={summary['review']['selected_candidates']}, "
        f"rejected={summary['review']['rejected_candidates']}"
    )
    print()
