...
DEFAULT_OUTPUT_ROOT = Path(__file__).resolve().parents[2] / "output" / "pipelines" / "strategy_archetype_showcase"
...
def run_example(*, output_root: Path | None = None, verbose: bool = True, reset_output: bool = True) -> ExampleArtifacts:
    ...
    with example_environment(workspace_root):
        for entry in archetypes:
            ...
            pipeline_id = f"m21_archetype_{strategy_name}"
            builder = (
    ...
    summary = {
        "pipeline": "strategy_archetype_showcase",
        "purpose": "Show trend, mean-reversion, and cross-sectional archetypes under one canonical composition framework.",
        "components": {
    ...
    if verbose:
        print("Strategy archetype showcase pipeline")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the canonical strategy archetype showcase pipeline.")
