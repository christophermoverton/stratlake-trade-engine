...
DEFAULT_OUTPUT_ROOT = Path(__file__).resolve().parent / "output" / "real_data_full_scenario_sweep_case_study"
...
def _resolved_config(output_root: Path) -> Any:
    ...
    base["portfolio"]["enabled"] = True
    base["candidate_selection"]["execution"]["enable_review"] = True
    base["review"]["filters"]["run_types"] = ["alpha_evaluation", "portfolio"]
    base["reuse_policy"] = {
        ...
        "include": [
            {
                ...
                "description": "Review-focused pass using sleeve metrics for alpha and portfolio outputs.",
                "overrides": {
                    "comparison": {"alpha_view": "sleeve"},
                    "review": {"filters": {"run_types": ["alpha_evaluation", "portfolio"]}},
                },
            }
        ],
    }
...
def _scenario_localized_config(config: Any, scenario_artifact_dir: Path) -> Any:
    payload = config.to_dict()
    stage_root = scenario_artifact_dir / "stage_outputs"
    payload["outputs"]["candidate_selection_output_path"] = (stage_root / "candidate_selection").as_posix()
    payload["outputs"]["portfolio_artifacts_root"] = (stage_root / "portfolios").as_posix()
    payload["outputs"]["comparison_output_path"] = (stage_root / "campaign_comparisons").as_posix()
    payload["outputs"]["review_output_path"] = (stage_root / "reviews" / "research_review").as_posix()
    payload["candidate_selection"]["output"]["path"] = (stage_root / "candidate_selection").as_posix()
    payload["candidate_selection"]["output"]["review_output_path"] = (
        stage_root / "reviews" / "candidate_review"
    ).as_posix()
    payload["review"]["output"]["path"] = (stage_root / "reviews" / "research_review").as_posix()
    return resolve_research_campaign_config(payload)


@contextmanager
def localized_scenario_stage_outputs() -> Iterator[None]:
    import src.cli.run_research_campaign as campaign_runner

    original_run_single = campaign_runner._run_single_research_campaign

    def wrapped(config: Any, *, scenario_context: Any | None = None) -> Any:
        effective_config = config
        if scenario_context is not None:
            effective_config = _scenario_localized_config(config, scenario_context.scenario_artifact_dir)
        return original_run_single(effective_config, scenario_context=scenario_context)

    with patch("src.cli.run_research_campaign._run_single_research_campaign", wrapped):
        yield
...
def run_example(
    *,
    ...
    with example_environment(), localized_scenario_stage_outputs():
    ...
    summary_payload: dict[str, Any] = {
        "workflow": [
            ...
            "localize_stage_output_roots_per_scenario_at_runtime",
            "run_full_campaign_orchestration_for_all_expanded_scenarios",
    ...
    if verbose:
        print("Real-Data Full Scenario Sweep Case Study")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a full-stage scenario-sweep campaign case study using repository features and existing alpha models."
    )
    parser.add_argument(
        "--output-root",
        help="Optional output root override. Defaults to docs/examples/output/real_data_full_scenario_sweep_case_study.",
    )
    parser.add_argument(
        "--no-reset",
        action="store_true",
