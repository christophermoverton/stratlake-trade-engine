...
def test_plot_artifact_helpers_return_standardized_run_scoped_paths() -> None:
    ...
    assert get_plot_path(run_dir, "rolling_sharpe_debug") == run_dir / "plots" / "rolling_sharpe_debug.png"
    assert get_plot_path(run_dir, "rolling_sharpe") == run_dir / "plots" / "rolling_sharpe_debug.png"
    assert get_plot_path(run_dir, "signal_distribution") == run_dir / "plots" / "signal_distribution_debug.png"
    assert get_plot_path(run_dir, "exposure_over_time") == run_dir / "plots" / "exposure_over_time_debug.png"
    assert get_plot_path(run_dir, "long_short_counts") == run_dir / "plots" / "long_short_counts_debug.png"
    assert get_plot_path(run_dir, "trade_return_distribution_debug") == (
        run_dir / "plots" / "trade_return_distribution_debug.png"
    )
    assert get_plot_path(run_dir, "trade_return_distribution") == (
        run_dir / "plots" / "trade_return_distribution_debug.png"
    )
    assert get_plot_path(run_dir, "win_loss_distribution_debug") == (
        run_dir / "plots" / "win_loss_distribution_debug.png"
    )
    assert get_plot_path(run_dir, "win_loss_distribution") == run_dir / "plots" / "win_loss_distribution_debug.png"
    ...
    assert get_plot_intent("equity_curve") == "report"
    assert get_plot_intent("drawdown") == "report"
    assert get_plot_intent("rolling_sharpe_debug") == "debug"
    assert get_plot_intent("trade_return_distribution") == "debug"
